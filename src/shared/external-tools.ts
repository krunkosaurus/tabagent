...
export const EXTERNAL_TOOLS = [
  ...
  { name: "scroll", description: "Scroll the document and report actual movement; if it has no scroll range, use the scrolling area at viewport center. Supply a snapshot ref to choose its scrollable ancestor. Default method 'dom' avoids wheel input; use 'wheel' for wheel-specific controls. Snapshot afterwards for newly loaded content. Follows the shared tab's approval setting.", readonly: false,
    parameters: object({ direction: { type: "string", enum: ["up", "down", "left", "right"] }, amount: number(0, 100_000), ref,
      method: { type: "string", enum: ["dom", "wheel"], description: "Default dom. Wheel completion does not confirm movement; never automatically retry a timed-out wheel." } }) },
